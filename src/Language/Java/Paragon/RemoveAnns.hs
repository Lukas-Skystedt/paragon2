...
module Language.Java.Paragon.RemoveAnns where
...
fRem :: (Functor f,  VoidAnn a) => f (a x) -> f (a UD)
fRem = fmap removeAnn
...
instance VoidAnn RefType where
  ...
  removeAnn (ArrayType _ t)  = ArrayType () (removeAnn t)
...
instance VoidAnn TypeArgument where
  removeAnn TypeArgumentExp{} = error "Type Arguments not supported in removeAnn"
