...
instance NotAppl VarInit where
  ...
  notAppl (InitArray _ arInit) = error "notAppl: case InitArray not implemented" --InitArray () (notAppl arInit)
...
-- Paragon specific
  ...
  notAppl (Reads   _ p ) = Reads   () (notAppl p) 
  notAppl (Writes  _ p ) = Writes  () (notAppl p) 
  notAppl (Opens   _ ls) = Opens   () (fRem ls)
  notAppl (Closes  _ ls) = Closes  () (fRem ls)
  notAppl (Expects _ ls) = Expects () (fRem ls) 
...
instance NotAppl Exp where
  notAppl (Lit _ lit)        = Lit Nothing (notAppl lit)
  notAppl (ClassLit _ typ)   = ClassLit Nothing (fRem typ)
  notAppl (This _)           = This Nothing
  notAppl (ThisClass _ name) = ThisClass Nothing (notAppl name)
  notAppl (Paren _ exp)      = Paren Nothing (notAppl exp)
  notAppl (InstanceCreation _ tArgs cType args cBody) =
    InstanceCreation Nothing (fRem tArgs) (notAppl cType) (fRem args)
      (fRem cBody)

  notAppl (QualInstanceCreation _ exp tArgs ident args cBody) =
    QualInstanceCreation Nothing (notAppl exp) (fRem tArgs) (notAppl ident)
      (fRem args) (fRem cBody)

  notAppl (ArrayCreate _ typ expnPols mPols) =
    ArrayCreate Nothing (notAppl typ)
      (map (\(a,b) -> (notAppl a,fRem b)) expnPols)
      (map (fRem) mPols)

  notAppl (ArrayCreateInit _ t pols aInit) =
    ArrayCreateInit Nothing (notAppl t) (map (fRem) pols)
      (notAppl aInit)

  notAppl (FieldAccess _ fAccess) = FieldAccess Nothing (notAppl fAccess)
  notAppl (MethodInv _ mInvoke)   = MethodInv Nothing (notAppl mInvoke)
  notAppl (ArrayAccess _ aIndex)  = ArrayAccess Nothing (notAppl aIndex)
  notAppl (ExpName _ name)        = ExpName Nothing (notAppl name)
  notAppl (PostIncrement _ exp)   = PostIncrement Nothing (notAppl exp)
  notAppl (PostDecrement _ exp)   = PostDecrement Nothing (notAppl exp)
  notAppl (PreIncrement  _ exp)   = PreIncrement Nothing (notAppl exp)
  notAppl (PreDecrement  _ exp)   = PreDecrement Nothing (notAppl exp)
  notAppl (PrePlus  _ exp)        = PrePlus Nothing (notAppl exp)
  notAppl (PreMinus _ exp)        = PreMinus Nothing (notAppl exp)
  notAppl (PreBitCompl _ exp)     = PreBitCompl Nothing (notAppl exp)
  notAppl (PreNot  _ exp)         = PreNot Nothing (notAppl exp)
  notAppl (Cast _ typ exp)        = Cast Nothing (notAppl typ) (notAppl exp)
  notAppl (BinOp _ e1 op e2)      =
    BinOp Nothing (notAppl e1) (notAppl op) (notAppl e2)
  notAppl (InstanceOf _ exp refT) =
    InstanceOf Nothing (notAppl exp) (notAppl refT)
  notAppl (Cond _ e1 e2 e3)       =
    Cond Nothing (notAppl e1) (notAppl e2) (notAppl e3)
...
instance NotAppl PolicyExp where
  notAppl (PolicyLit     _ cs) = PolicyLit () (fRem cs)
  notAppl (PolicyOf      _ i) = PolicyOf () (notAppl i)
  notAppl (PolicyThis    _) = PolicyThis ()
  notAppl (PolicyTypeVar _ i) = PolicyTypeVar () (notAppl i)

instance NotAppl Clause where
  notAppl (Clause _ cvds ch a) = Clause () (fRem cvds) (notAppl ch) (fRem a)
