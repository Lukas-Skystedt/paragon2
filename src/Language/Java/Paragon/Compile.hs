...
compileInterfaceDecl :: InterfaceDecl T -> PcsInterfaceDecl
...
compileInterfaceBody :: InterfaceBody T -> PcsInterfaceBody
...
-- 3. Transform body
compileClassDecl :: ClassDecl T -> PcsClassDecl
...
splitTypeParams :: [TypeParam T]
                -> ([PcsTypeParam],[PcsMemberDecl],[PcsFormalParam],[PcsBlockStmt])
...
compileClassBody :: ClassBody T -> [PcsMemberDecl] -> [PcsFormalParam] -> [PcsBlockStmt] -> PcsClassBody
...
compileDecl :: [PcsFormalParam] -> [PcsBlockStmt] -> Decl T -> [PcsDecl]
...
compileMemberDecl :: [PcsFormalParam] -> [PcsBlockStmt] -> MemberDecl T -> [PcsDecl]
compileMemberDecl tpPars tpAsss md =
    case md of
      LockDecl {} -> compileLockDecl md
      _ -> (:[]) . PcsMemberDecl $ compileSimpleMemberDecl tpPars tpAsss md

compileSimpleMemberDecl :: [PcsFormalParam] -> [PcsBlockStmt] -> MemberDecl T -> PcsMemberDecl
...
compileConstrBody :: [PcsBlockStmt] -> ConstructorBody T -> PcsConstructorBody
...
compileECI :: ExplConstrInv T -> PcsExplConstrInv
...
compileExn :: ExceptionSpec T -> PcsExceptionSpec
compileExn (ExceptionSpec _ _ms rt) = PcsExceptionSpec [] -- no modifiers on exceptions in java!
                                       $ compileRefType rt

compileReturnType :: ReturnType T -> PcsReturnType
...
compileRefType :: RefType T -> PcsRefType
...
compileClassType :: ClassType T -> (PcsClassType, [PcsArgument])
...
compileClassType_ :: ClassType T -> PcsClassType
...
compileVarDeclGeneric :: ([PcsModifier] -> PcsType -> [PcsVarDecl] -> res)
...
compileForInit :: ForInit T -> PcsForInit
...
compileMethodInv :: MethodInvocation T -> PcsMethodInvocation
...
splitTypeArgs :: [TypeArgument T] -> ([PcsTypeArgument], [PcsArgument])
splitTypeArgs = go ([], [])
    where
...
splitNWTypeArgs :: [NonWildTypeArgument T] -> ([PcsNonWildTypeArgument], [PcsArgument])
splitNWTypeArgs = go ([], [])
    where
      go (ttas, as) [] = (reverse ttas, reverse as)
      go (ttas, as) (nwta:tas) =
...
compileBinOp :: PcsOp -> Exp T -> Exp T -> PcsExp
...
mkParagonPolicyOp :: PcsOp -> PcsExp -> PcsExp -> PcsExp
...
mkParagonPolicyAssign :: PcsAssignOp -> PcsLhs -> PcsExp -> PcsExp
...
compileAOp :: PcsAssignOp -> Lhs T -> Exp T -> PcsExp
...
compileArrayIndex :: ArrayIndex T -> PcsArrayIndex
compileArrayIndex (ArrayIndex _ eArr eI)
    = PcsArrayIndex (compileExp eArr) (compileExp eI)

compileFieldAccess :: FieldAccess T -> PcsFieldAccess
...
compilePolicyExp :: PolicyExp T -> PcsExp
...
headToExp, atomToExp :: [(PcsIdent, Int)] -> Atom T -> PcsExp
...
actorToExp :: [(PcsIdent, Int)] -> Actor T -> PcsExp
...
compileLockDecl :: MemberDecl T -> [PcsDecl]
compileLockDecl md =
    case md of
...
lockModToExp :: Ident T -> Modifier T -> PcsExp
