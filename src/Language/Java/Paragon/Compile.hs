...
import Language.Java.Paragon.RemoveAnnsTTG
...
compileInterfaceDecl :: InterfaceDecl PA -> InterfaceDecl ()
...
compileInterfaceBody :: InterfaceBody PA -> InterfaceBody ()
...
-- 3. Transform body
compileClassDecl :: ClassDecl PA -> ClassDecl ()
...
compileClassBody :: ClassBody PA -> [MemberDecl ()] -> [FormalParam ()] -> [BlockStmt ()] -> ClassBody ()
...
compileDecl :: [FormalParam ()] -> [BlockStmt ()] -> Decl PA -> [Decl ()]
...
compileMemberDecl :: [FormalParam ()] -> [BlockStmt ()] -> MemberDecl PA -> [Decl ()]
compileMemberDecl tpPars tpAsss md =
    case md of
      LockDecl {} -> compileLockDecl md
      _ -> (:[]) . MemberDecl () $ compileSimpleMemberDecl tpPars tpAsss md

compileSimpleMemberDecl :: [FormalParam ()] -> [BlockStmt ()] -> MemberDecl PA -> MemberDecl ()
...
compileConstrBody :: [BlockStmt ()] -> ConstructorBody PA -> ConstructorBody ()
...
compileECI :: ExplConstrInv PA -> ExplConstrInv ()
...
compileExn :: ExceptionSpec PA -> ExceptionSpec ()
compileExn (ExceptionSpec _ _ms rt) = ExceptionSpec () [] -- no modifiers on exceptions in java!
                                       $ compileRefType rt

compileReturnType :: ReturnType PA -> ReturnType ()
...
compileRefType :: RefType PA -> RefType ()
...
compileClassType :: ClassType PA -> (ClassType (), [Argument ()])
...
compileClassType_ :: ClassType PA -> ClassType ()
...
compileForInit :: ForInit PA -> ForInit ()
...
compileMethodInv :: MethodInvocation PA -> MethodInvocation ()
...
splitTypeArgs :: [TypeArgument PA] -> ([TypeArgument ()], [Argument ()])
splitTypeArgs = go ([], [])
    where
...
splitNWTypeArgs :: [NonWildTypeArgument PA] -> ([NonWildTypeArgument ()], [Argument ()])
splitNWTypeArgs = go ([], [])
    where
      go (ttas, as) [] = (reverse ttas, reverse as)
      go (ttas, as) (nwta:tas) =
...
compileBinOp :: Op () -> Exp PA -> Exp PA -> Exp ()
...
compileAOp :: AssignOp () -> Lhs PA -> Exp PA -> Exp ()
...
compileArrayIndex :: ArrayIndex PA -> ArrayIndex ()
compileArrayIndex (ArrayIndex _ eArr eI)
    = ArrayIndex () (compileExp eArr) (compileExp eI)

compileFieldAccess :: FieldAccess PA -> FieldAccess ()
...
compilePolicyExp :: PolicyExp PA -> Exp ()
...
clauseHeadToExp :: [(Ident (), Int)] -> ClauseHead PA -> Exp ()
...
headToExp, atomToExp :: [(Ident (), Int)] -> Atom PA -> Exp ()
...
actorToExp :: [(Ident (), Int)] -> Actor PA -> Exp ()
...
compileLockDecl :: MemberDecl PA -> [Decl ()]
compileLockDecl md =
    case md of
...
lockExpsToInit :: Ident PA -> [Exp ()] -> [Decl ()]
lockExpsToInit _ [] = []
lockExpsToInit _i es = [InitDecl () True . Block () $
                        map (BlockStmt () . ExpStmt ()) es]

lockPropToExp :: Ident PA -> LClause PA -> Exp ()
...
lockModToExp :: Ident PA -> Modifier PA -> Exp ()
